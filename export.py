...
def export_onnx(model, im, file, opset, train, dynamic, simplify, prefix=colorstr('ONNX:')):
    ...
    torch.onnx.export(
        ...
        dynamic_axes={
            ...
            'bbox': {
                0: 'batch',
                1: 'anchors'},  # shape(1,25200, 1, 4)
            'cls_score': {
                0: 'batch',
                1: 'anchors'}    # shape(1,25200, 80)
    ...
    if simplify:
        ...
        model_onnx, check = onnxsim.simplify(model_onnx,
                                                # not needed any more, onnxsim can now support dynamic input shapes natively
                                                #dynamic_input_shape=dynamic,
                                                # overwrite_input_shapes={'images': list(im.shape)} if dynamic else None
                                                )
    ...
    # If set to true, the boxes input are shared across all classes.
    ...
    # nms参数
    nms_attrs["scoreThreshold"] = 0.25
    nms_attrs["iouThreshold"] = 0.45
    # 控制plugin输入输出的bbox数量
    nms_attrs["topK"] = 2*300
    nms_attrs["keepTopK"] = 300
    nms_attrs["numClasses"] = 80
    # yolov5 detect层输出的bbox坐标已经scale到feature map尺寸的所以这里要设置为0
